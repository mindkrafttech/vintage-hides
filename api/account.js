...
// OTP flow uses MSG91 OTP API — MSG91 generates + verifies the code automatically
...
function makeToken(customer) {
  return jwt.sign(
    ...
    process.env.CUSTOMER_JWT_SECRET || process.env.JWT_SECRET || process.env.ADMIN_JWT_SECRET,
    { expiresIn: '30d' }
  );
}
...
function getCustomer(req) {
  ...
  try { return jwt.verify(h.slice(7), process.env.CUSTOMER_JWT_SECRET || process.env.JWT_SECRET || process.env.ADMIN_JWT_SECRET); } catch { return null; }
}
